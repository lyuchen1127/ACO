...
void Antteam_Choice_Next(solution_Ant_team* Ant_team_temp, double tao[CityCount][CityCount], double alldisarray[CityCount][CityCount], read_city RC[CityCount])
{
	...
	for (int i = 0; i <= CarCount; i++)
	{
		for (int j = 0; j < CityCount; j++)
		{
			if (Ant_team_temp->ant_team[i].can_visit[j] == false)
			{
				...
				Ant_team_temp->ant_team[i].select_probability[j] = tao[Ant_team_temp->ant_team[i].tour[Ant_team_temp->ant_team[i].tour_citycount - 1]][j]
