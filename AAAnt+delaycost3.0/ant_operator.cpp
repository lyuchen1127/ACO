...
void Ant_Next_City(solution_Ant* Ant_temp,double tao[CityCount][CityCount],double Delttao[CityCount][CityCount], double alldisarray[CityCount][CityCount])
{
	...
	for (int i = 0; i < CityCount; i++)
	{
		if (Ant_temp->can_visit[i] == true)
			temp += pow((tao[currentcity][i]),alf) * pow(1.0 / alldisarray[currentcity][i], beta);
	}
	for (int i = 0; i < CityCount; i++)
	{
		...
		{
			Ant_temp->select_probability[i] = pow((tao[currentcity][i]),alf) * pow(1.0 / alldisarray[currentcity][i], beta) / temp;//对于未到过的城市，则计算选择概率
	...
	/*if (nextcity == -1)
	{
		...
		{
			if (can_visit[i] == true)
				if (temp<pow((Map_City.tao[currentcity][i]),alf)*pow(1.0 / Map_City.Distance[currentcity][i], beta))
				{
					temp = pow((Map_City.tao[currentcity][i]),alf)*pow(1.0 / Map_City.Distance[currentcity][i], beta);
					nextcity = i;
				}
		}
	}*/
...
void Ant_Build_Trip(solution_Ant* Ant_temp, double tao[CityCount][CityCount], double Delttao[CityCount][CityCount], double alldisarray[CityCount][CityCount])
{
	for (int i = 0; i < CityCount; i++)
	{
		Ant_temp->can_visit[i] = true;
	...
	{
		if (Ant_temp->tour_citycount < CityCount) Ant_Next_City(Ant_temp, tao, Delttao, alldisarray);//轮盘赌法选择下一个城市
