...
for (int j = 0; j < CarCount; j++)
	...
	{
		double tao[CityCount][CityCount];//信息素矩阵
		double Delttao[CityCount][CityCount];//信息素变化量矩阵
		for (int i = 0; i < CityCount; i++)
		{
			for (int j = 0; j < CityCount; j++)
			{
		...
		for (int iteratorcishu = 0; iteratorcishu < MAX_GEN; iteratorcishu++)
		{
			...
			{
				Ant_Clear(&ant[k]);
				Ant_Build_Trip(&ant[k], tao, Delttao, alldisarray);
			...
			UpdatePheromones(tao, Delttao, ant);
			for (int i = 0; i < AntCount; i++)
			{
				Ant_Clear(&ant[i]);
			}
		...
		//	UpdatePheromones_team(tao, Delttao, best_ant_t);
		//}
		...
		while (count <= MAX_GEN + 100)
		{
			...
			for (int k = 0; k < CityCount; k++)
			{
				for (int i = 0; i < CityCount; i++)//判断所有点是否走完
				{
					...
					{
						Antteam_Choice_Next(&sol_ant_team[k], tao, alldisarray, RC);
					}
				}
				...
				for (int j = 0; j <= CarCount; j++)
				{
					...
					//UpdatePheromones_team(tao, Delttao, sol_ant_team[k]);
				}
				UpdatePheromones_team(tao, Delttao, sol_ant_team[k]);
		...
		cout << best_ant_t.ant_team_totallen << "          " << best_ant_t.total_delay << "      " << (best_ant_t.ant_team_totallen * 0.8*(max_dis/1500.0) + best_ant_t.total_delay*(max_dis / 1500.0) * 0.2* (max_dis / 1500.0)) << endl;
	}
	return 0;
}
